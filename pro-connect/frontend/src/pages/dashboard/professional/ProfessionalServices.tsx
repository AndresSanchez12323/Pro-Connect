  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/dashboard/professional/create-service" className="text-xs bg-primary text-black font-bold px-4 py-2 rounded-sm hover:bg-green-400 transition-colors uppercase font-mono">
          + Nuevo Servicio
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {services.map((service) => (
            <div key={service.id} className="minimal-card p-6 border-l-4 border-l-primary/50 group hover:border-l-primary transition-all">
              <div className="flex justify-between items-start mb-4 gap-4">
                <h3 className="text-lg font-bold text-white font-mono">{service.name}</h3>
                <span className="text-xs font-bold text-green-400 bg-green-400/10 px-2 py-1 rounded border border-green-400/20">ACTIVO</span>
              </div>
              <p className="text-sm text-gray-400 mb-4">Modalidad: {traducirModalidad(service.mode)}</p>
              <div className="flex justify-between items-center pt-4 border-t border-white/5 text-xs font-mono text-gray-500">
